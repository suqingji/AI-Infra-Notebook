...
“内存交互通过PCIe总线通信”是传统异构架构的核心事实，但这同时也是高性能计算（HPC）中最头疼的**性能瓶颈**。

**数据搬移成本：** 主存（Host Memory）与显存（Device Memory）是物理隔离的。在计算开始前，CPU必须通过PCIe总线将数据拷贝到GPU（H2D）；计算完成后，又得通过PCIe将结果拷回主存（D2H）。

**开发者的挑战：** 在使用C\+\+或CUDA编写底层求解器时，优化的核心工作之一就是尽可能将数据“驻留”在GPU显存中，减少不必要的PCIe往返通信，因为总线的带宽远远低于GPU内部计算的吞吐量。



1. CUDA Runtime API（运行时 API）：主打“开箱即用”

- **命名规律：** 函数统一以 `cuda` 开头，例如 `cudaMalloc`（分配显存）、`cudaMemcpy`（数据拷贝）。

- **工作机制（自动挡）：** 它的核心优势是**隐式初始化**和**自动上下文（Context）管理**。当你在代码中调用第一个 `cuda` 函数时，Runtime 会自动在后台为你初始化 GPU，并为当前的 CPU 线程绑定一个 GPU 上下文。开发者根本不需要去管“上下文”是个什么东西。

- **使用场景：** 在绝大多数情况下，当我们使用 C\+\+ 编写并行求解器、处理网格数据或是常规的异构加速计算时，写在 `.cu` 文件里并通过 `nvcc` 编译器编译的，都是 Runtime API。它足够高效且易于维护，能满足 95% 以上的开发需求。



2. CUDA Driver API（驱动 API）：主打“绝对控制权”

- **命名规律：** 函数统一以 `cu` 开头，例如 `cuMemAlloc`、`cuCtxCreate`。

- **工作机制（手动挡）：** 一切都需要显式、手动地完成。在使用任何 GPU 资源前，你必须先手动调用 `cuInit` 初始化驱动，然后手动创建设备句柄，手动创建和管理上下文（Context），最后才能分配内存。

- **使用场景：** 既然这么麻烦，为什么还需要它？因为它提供了最底层的控制力。

    - **动态加载：** 它可以直接加载和执行 JIT（即时编译）生成的 PTX 汇编代码或 `.cubin` 二进制文件，而不需要依赖 `nvcc` 静态编译。

    - **框架底层：** 当开发需要极细粒度资源调度的系统时（比如构建深度学习框架的底层算子调度、物理信息神经网络的定制化训练引擎，或实现复杂的语言互操作性），开发者会深入到 Driver API，以压榨最后一滴性能和内存。
...
// 这里的 __global__ 是 CUDA C 扩展的**执行空间限定符（Execution Space Specifier）**。
// **它的含义是：** 这是一个**内核函数（Kernel）**。它在主机（CPU）端被调用，但在设备（GPU）端执行。
// **执行方式：** 习惯了 OpenMP 或 MPI 的开发者会发现，这里的代码并没有写任何显式的循环或线程 ID 分配逻辑。这就是 CUDA 的 **SIMT（单指令多线程）** 模型。这个函数就像一个独立的任务模板，当它被发射到 GPU 时，成百上千个线程会**同时且独立**地执行函数体内的这句话。

int main(void)
{
    hello_from_gpu<<<4, 4>>>();
    
    // 这行代码是这段程序的心脏，尖括号 <<<GridSize, BlockSize>>> 是 CUDA 独有的**执行配置语法**。CPU 就是通过它来决定派发多少个“工人”去执行任务。
    // **第一个参数 4 (Grid Size / 块数量)：** 告诉 GPU，将任务分配到 4 个线程块（Block）中。
    // **第二个参数 4 (Block Size / 线程数量)：** 告诉 GPU，每个线程块里面包含 4 个线程（Thread）。
    // **总工作量：** 4 个 Block × 4 个 Thread/Block = **16 个线程**。这意味着 GPU 会并行地执行 16 次 printf。

    cudaDeviceSynchronize();

    // 这是极其关键的一步，也是初学者最容易踩坑的地方。
    // **异步发射：** 在 CUDA 中，CPU 调用内核函数 hello_from_gpu<<<4, 4>>>(); 的操作是异步（Asynchronous）的。这意味着 CPU 下达完指令后，根本不会等待 GPU 执行完毕，就会以极低的延迟瞬间返回，并继续执行下一行代码。
    // **如果去掉这行代码：** CPU 瞬间发射完任务后，立刻执行到了 return 0;，导致整个 C 程序（Host 进程）直接结束。此时 GPU 可能才刚刚收到指令，还没来得及向缓冲区写入字符，程序就被操作系统杀死了，终端上将没有任何输出。
    // **它的作用：** 设置一个显式的**屏障（Barrier）**。强制 CPU 在这里停下来挂起，直到 GPU 上的所有线程都执行完毕（并把 printf 的缓冲数据传回主机），CPU 才会继续往下走。
...
前面一节代码即使用cuda编译器编译，仍旧是cuda编译器调用的c\+\+编译器部分，因此并没有调用到gpu，现在尝试通过核函数控制GPU进行工作
...
核函数不支持C++的iostream，必须使用printf这个函数
